...
export async function startHttpSse(port = Number(process.env.PORT || 3000)): Promise<void> {
  const app = express();
  app.use(cors());
  ...
  app.get('/mcp', async (req, res) => {
    ...
    console.log(
      `[sse] session created: ${sessionId}. Active sessions: ${Object.keys(transports).length}`,
    );
    transport.onclose = () => {
      delete transports[sessionId];
      console.log(
        `[sse] session closed: ${sessionId}. Active sessions: ${Object.keys(transports).length}`,
      );
    };
    const server = createMcpServer();
    await server.connect(transport);
  ...
  app.post('/messages', async (req, res) => {
    ...
    if (!transport) {
      console.warn(
        `[sse] POST /messages session not found: ${sessionId}. Active: ${Object.keys(transports).join(',')}`,
      );
